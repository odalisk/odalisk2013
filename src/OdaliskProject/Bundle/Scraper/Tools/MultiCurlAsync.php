...
class MultiCurlAsync extends Curl
{
        ...
        do {
            ...
            while ($done = curl_multi_info_read($this->master)) {
                ...
                if (is_callable($callback)) {
                    $this->fromString($this->getLastResponse(curl_multi_getcontent($done['handle'])),$response);
    ...
    private function fromString($raw,$resp)
    {
        ...
        for ($i = 0; $i < count($lines); $i += 2) {
            ...
            if (empty($line)) {
                $resp->setContent(implode('', array_slice($lines, $i + 2)));
                break;
            } else {
                $resp->addHeader($line);
            }
    ...
    private function getLastResponse($raw)
    {
